  ...
  return (
    <div>
      ...
      <div className="relative inline-block w-full text-gray-700">
        <select
          {...selectProps}
          className="w-full appearance-none bg-monika-gray bg-opacity-10 text-white rounded-md shadow-md py-3 px-5 pr-8 text-sm sm:text-lg placeholder-gray-400 h-full">
          {children}
        </select>
        <div className="absolute inset-y-0 right-0 mr-1 flex items-center px-2 pointer-events-none">
          <svg
            className="w-4 h-4 fill-current text-monika-gray"
            viewBox="0 0 20 20">
            <path
...
export function SelectOption({
  ...
  return (
    <option className="text-black" {...selectOptionProps}>
      {children}
    </option>
  );
}
